...
with st.sidebar:
    ...
    if st.button("🏠 Beranda", use_container_width=True):
        ...
        st.session_state.prompt_tab = "lihat"
        st.rerun()
...
# --- LANGKAH 3: HASIL PROMPT ---
    ...
    # --- PROMPT INGGRIS (Lihat / Salin / Edit) ---
    prompt_en = st.session_state.edited_prompt or result.get("positive", "")
    prompt_neg = result.get("negative", "")
    prompt_tab = st.session_state.get("prompt_tab", "lihat")

    st.markdown(f"### 🇬🇧 Prompt Inggris (untuk Veo 3) — `{len(prompt_en)} karakter`")

    col1, col2, col3 = st.columns([1, 1, 6])
    with col1:
        if st.button("📋 Lihat", use_container_width=True, type="secondary" if prompt_tab != "lihat" else "primary"):
            st.session_state.prompt_tab = "lihat"
            st.rerun()
    with col2:
        if st.button("📋 Salin", use_container_width=True, type="secondary" if prompt_tab != "salin" else "primary"):
            st.session_state.prompt_tab = "salin"
            st.rerun()
    with col3:
        if st.button("✏️ Edit", use_container_width=True, type="secondary" if prompt_tab != "edit" else "primary"):
            st.session_state.prompt_tab = "edit"
            st.rerun()
    ...
    elif prompt_tab == "salin":
        st.code(prompt_en, language="text", line_numbers=True)
        escaped = prompt_en.replace("`", "\\`").replace("$", "\\$").replace("${", "\\${")
        html = f"""
        <div style="display:flex; justify-content:center; margin:10px 0;">
            <button onclick="navigator.clipboard.writeText(`{escaped}`).then(() => {{this.innerHTML='✅ Tersalin!'; setTimeout(()=>{{this.innerHTML='📋 Salin Prompt ke Clipboard';}},2000)}})"
            style="padding:12px 24px; font-size:16px; cursor:pointer; background:#4CAF50; color:white; border:none; border-radius:8px;">
